...
module.exports = {
  insert (transactionCode, storeId) {
    // insert a new row
    const sql = `INSERT INTO ${dbTable} (transaction_code, store_id) VALUES (?, ?)`
    return dbRunPromise(sql, [ transactionCode, storeId ])
  },
  ...
  getStore (transactionCode) {
    // find store for given PagHiper transaction code
    ...
    return new Promise((resolve, reject) => {
      db.get(sql, [ transactionCode ], (err, row) => {
        if (err) {
          ...
          // found with success
          // resolve the promise returning respective Store ID
          resolve(row.store_id)
        } else {
