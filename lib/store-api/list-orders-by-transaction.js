...
module.exports = ({ appSdk, storeId, auth }, transactionCode, intermediatorCode) => {
  ...
  let url = '/orders.json?transactions.intermediator.transaction_id=' + transactionCode
  if (intermediatorCode) {
    url += '&transactions.app.intermediator.code=' + intermediatorCode
  }
  url += '&fields=_id,transactions._id,transactions.intermediator,transactions.status'
  // logger.log(url)
  const method = 'GET'
  const data = null
